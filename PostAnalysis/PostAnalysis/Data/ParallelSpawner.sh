...
function CreateBatches_Local
{
  ...
  echo "cd ../PostAnalysis && asetup AnalysisBase,21.2.58,here" >> Spawn.sh
  echo "cd ../build" >> Spawn.sh
  echo "cmake ../PostAnalysis" >> Spawn.sh
  echo "make" >> Spawn.sh
...
do
  ...
  do
    ...
    do
      ...
      mkdir build
      cp $root_dir/Merged_MC.root ./build/Merged_MC.root
      cp -r $PostAnalysis_root_dir . 
      LJE=$L"_"$E
      
      CreateBatches_Local $LJE $M
