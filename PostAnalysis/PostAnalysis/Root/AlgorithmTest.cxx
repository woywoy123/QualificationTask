...
void TestAlgorithmMonteCarlo()
{
  ...
  for (M x = Output.begin(); x != Output.end(); x++)
  {
    ...
    std::map<TString, std::vector<TH1F*>> res = MainAlgorithm(Outside_JetCore, Data, Params, 3); 
    ...
    std::vector<TH1F*> trk1_r = res["Result1"]; 
    std::vector<TH1F*> trk2_r = res["Result2"]; 
    std::vector<TH1F*> trk3_r = res["Result3"]; 
    //std::vector<TH1F*> trk4_r = res["Result4"];
...
void ProcessMonteCarloResults()
{
  ...
  for (it p = Map.begin(); p != Map.end(); p++)
  {
    ...
    std::vector<TH1F*> Track2_P = {Pred[0+trks], Pred[1+trks], Pred[2+trks]}; //, Pred[3+4]}; 
    std::vector<TH1F*> Track3_P = {Pred[0+trks*2], Pred[1+trks*2], Pred[2+trks*2]}; //, Pred[3+8]}; 
