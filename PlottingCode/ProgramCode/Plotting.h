...
TLegend* GenerateLegend(std::vector<TH1D*> Hist_V, TCanvas* can, float x_size = 0.9, float x_position = 0.5, float box_size = 0.89, float size = 0.8);
...
void CombinedGraphs(std::vector<TGraph*> gr, TString title, float ymin, float ymax, float xmin, float xmax, TString yname, TString xname, TCanvas* can, bool Log = false);
